...
wget http://megalotto.pl/wyniki/ekstra-pensja/losowania-z-roku-2017 -q -O - | sed -n -e '/lista_ostatnich_losowan"/,$p'  > lottohtml

# tworzenie pliku z numerami
grep -o 'list " >[0-9]\{1,2\}' ./lottohtml | sed 's|[^0-9]||g' | tr '\n' ',' | xargs -n 5 -d, | sed 's/ /,/g' > lottonumery
grep -o 'list"><strong>[0-9]\{1,2\}' ./lottohtml | sed 's|[^0-9]||g' > lottoplus
